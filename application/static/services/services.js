  ...
  var loadUserRecs = function(userid) {
    ...
    .then(function(resp) {
      ...
      resp.data.personal.forEach(function(item) {
        ...
        item.product_name = item.product.product_ame;
  ...
  var getSites = function() {
    ...
    if (Auth.userData.userid) {
      ...
      .then(function(resp) {
        ...
        resp.data.sites.forEach(function(item) {userSites.unshift(item);});
        return resp.data;
      });
    }
  };
  ...
  var addSite = function(site) {
    ...
    .then(function(resp) {
      ...
      if (resp.data.image === '' || undefined) {
       resp.data.image = '/photos/' + photoOptionsBlogs[Math.floor(Math.random()*photoOptionsBlogs.length)];
      }
      return resp.data;
    });
  };
