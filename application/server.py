...
def newUser():
    ...
    if not u_ctrl.user_exists(body['username']):
        #add user to db
        u_ctrl.make_new_user(str(datetime.datetime.now()), body.get('username'), body.get('password'), body.get('email'), body.get('name_title'), body.get('name_first'), body.get('name_last'), body.get('gender'), body.get('location'), body.get('birthday'), )
        #make response
        response = jsonify({'userid': u_ctrl.get_user_id(body['username'])})
